...
# =====================================================
#  POST - UPSERT de nota desde Cargar Notas
# =====================================================
...
def upsert_nota(
    ...
    try:
        if nota_db:
            # ACTUALIZAR: 
            # nota_db.nota es el campo de tu BASE DE DATOS
            # payload.valor es el campo de tu ESQUEMA/FRONTEND
            ...
            nueva_nota = models.Nota(
                ...
                nota=payload.valor  # <--- CAMBIO AQUÍ: payload.valor
            )
            db.add(nueva_nota)
        
        db.commit()
        return {"status": "success", "message": "Nota procesada correctamente"}

    except Exception as e:
        db.rollback()
        # Este print es vital: mira la consola negra de tu servidor cuando falle
